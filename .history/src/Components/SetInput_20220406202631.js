...
const correctDuration = (durationString) => {
    ...
    if (minutes >= 60)
    {
        ...
        console.log('morehours', additionalHours);
    ...
    //dispatch(update_set({ id: setId, changes: { duration: duration } }))
}

const transformNumber = (newValue, setDuration) => {
    ...
    if (newValue.length === 6 && number.length === 5)
    {
        newValue = newValue.replace(/:/g, '');
        newValue = [newValue.slice(0, 1), ':', newValue.slice(1)].join('');
    ...
    if (newValue.length === 6 && number.length === 7)
    {
        newValue = newValue.replace(/:/g, '');
        newValue = [newValue.slice(0, 2), ':', newValue.slice(2)].join('');
    }

    if (newValue.length === 4 && number.length === 5)
    {
        newValue = newValue.replace(/:/g, '');
        newValue = [newValue.slice(0, 1), ':', newValue.slice(1)].join('');
    }

    if (newValue.length === 3 && number.length === 4)
    {
        ...
        {exerciseType === 'Cardio' 
            ...
            :
                <TextInput 
                    ...
                    selectTextOnFocus
                    onChangeText={(text) => transformNumber(text, setDuration)}
                    onBlur={() => correctDuration(duration) }
                    value={duration}
                    ...
                    //     // <MaskedTextInput
                    //     // {...props}
                    //     // mask={ duration.length < 2 ? '00:09' : duration.length < 3 ? '00:99' : duration.length < 4 ? '0:09:99' : duration.length < 5 ? '00:99:99' : '09:99:99'}
                    //     // />
                    // }
                />
            }
