...
const SetInput = ({ setId }) => {
    ...
    const set = useSelector(selectSetById);
    ...
    return ( 
        <>
            <TextInput 
                ...
                onBlur={() => {dispatch(update_set({_id: setId, change: { weight }}))}}
                value={weight}
            />
            <TextInput 
                style={styles.metricsInput} 
                ...
                onBlur={() => {dispatch(update_set({_id: setId, change: { reps }}))}}
                value={reps}
            />
            <IconButton icon={isComplete ? 'checkbox' : 'square-outline'}  size={20} onPress={() => {setIsComplete(!isComplete)}}/>       
        </>
    );
};
