...
const isStateSet = ({ name, exerciseType, muscles, equipment, force }) => {
    ...
    if ( String.isEmpty(name) || String.isEmpty(exerciseType) || String.isEmpty(equipment) || String.isEmpty(force) || muscles.length == 0)
    {
        return false;
    }
    return true;
    ...
    return (
        <>
            ...
            <FAB 
                ...
                icon='ios-add'
                //disabled={true}
                onPress={(state) => {isStateSet(state) ? console.log('All Good!') : console.log('Not Good!')}}
            />
        </>
    );
