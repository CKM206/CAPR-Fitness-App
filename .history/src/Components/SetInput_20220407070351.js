...
const SetInput = ({ setId, exerciseType }) => {
    ...
    const set = useSelector(selectSetById(setId));
    console.log('IM GETTING HERE:', set)
    // State Management
    const [reps, setReps] = useState(set ? set.reps.toString() : '');
    const [weight, setWeight] = useState(set ? set.weight.toString() : '');
    const [duration, setDuration] = useState(set ? set.duration : '');
    const [distance, setDistance] = useState(set ? set.distance : '');
