...
import {add_set, addSetToExercise, add_new_set} from '../redux/features/workout/workoutSetsSlice';
import { selectAllExercises, selectExerciseById } from '../redux/features/exercises/exercisesSlice';
import { selectAllSets, selectSetsOfExercise } from '../redux/features/workout/workoutSetsSlice';
...
const WorkoutExerciseContainer = ({ workoutExercise }) => {
    ...
    const sets = useSelector((state) => selectSetsOfExercise(state, workoutExercise ))
    const currentNumberOfSets = sets.length;

    //console.log('Current Number of Sets: ', currentNumberOfSets);
    useMemo(() => {
                    ...
                    ListFooterComponent={
                        <>
                            <Button labelStyle={{fontSize: 12}} compact onPress={(state) => {
                                dispatch(add_new_set({_id: currentNumberOfSets, reps: 0, weight: 0, duration: 0, restTimer: 0, isComplete: false, exercise_id: workoutExercise._id }))
                            }}>
                            Add Set
                            </Button>
                        </>
