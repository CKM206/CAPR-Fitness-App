...
const ProcessWorkout = (activeWorkout, workoutExercises, isASetCompleted) => {

    // Check for at Least 1 Exercise


    // Check for at Least 1 Complete Set
    console.log('\n\nCan Finish?: ', isASetCompleted);
...
const WorkoutDetailScreen = ({ navigation, route }) => {
    ...
    const [isASetCompleted, setIsASetComplete] = useState(false);
    ...
    const exercisesFooterComponent = () => {
        return (
            <>
                ...
                <Button
                    onPress={() => ProcessWorkout(activeWorkout, workoutExercises, isASetCompleted)}
                >
                    Finish!
                </Button>
            </>
        ...
        <View style={styles.screen}>
            <FlatList 
                ...
                renderItem={({ item }) => {
                    if (item) {
                    return (                 
                        <WorkoutExerciseContainer workoutExercise={item} workoutSetState={setIsASetComplete} />
