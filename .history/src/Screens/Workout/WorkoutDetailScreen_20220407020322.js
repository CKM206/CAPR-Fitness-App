...
const ProcessWorkout = (activeWorkout, workoutExercises, completedSets) => {

    // Check for at Least 1 Exercise
    if (workoutExercises.length > 0 && completedSets.length > 0)
    {
        console.log('YUP');
    }
    else{
    ...
    const exercisesFooterComponent = () => {
        return (
            <>
                ...
                <Button
                    onPress={() => ProcessWorkout(activeWorkout, workoutExercises, completedSets)}
                >
                    Finish!
                </Button>
            </>
