...
const addExercise = (exercise) => {
    ...
    let nextIndex = updatedExercises.length + currentExercisesNumber - 1;
...
const removeExercise = (item) => {
    ...
    updatedExercises.splice(updatedExercises.indexOf(item), 1);
            ...
            renderItem={({ item }) => {
                return (
                    <TouchableOpacity onPress={() => { 
                        let chosenExercise = chosenExercises.filter((exercise) => exercise.exercise_id === item._id).length > 0
