...
const removeExercise = (item) => {
    ...
    const exerciseForRemoval = updatedExercises.filter((exercise) => exercise.exercise_id === item._id)

    console.log('Remove this item');
    console.log(item);
    // Remove the element at the elements index
    updatedExercises.splice(chosenExercises.indexOf(item._id), 1);
            ...
            renderItem={({ item }) => {
                return (
                    <TouchableOpacity onPress={() => { 
                        let chosenExercise = chosenExercises.filter((exercise) => exercise.exercise_id === item._id);
