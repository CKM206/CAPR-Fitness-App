...
const ProcessWorkout = (activeWorkout, workoutExercises, dispatch) => {

    const exerciseSets = [];
    const thisExerciseSets = useSelector((state) => selectSetsOfExercise(state, workoutExercises[0]))
    workoutExercises.forEach((exercise) => {
        //exerciseSets.push(useSelector((state) => selectSetsOfExercise(state, exercise)))
    });
    console.log('\n\nActive Workout: ', activeWorkout);
    console.log('Workout Exercises: ', thisExerciseSets);
}
    ...
    const exercisesFooterComponent = () => {
        return (
            <>
                ...
                <Button
                    onPress={() => ProcessWorkout(activeWorkout, workoutExercises, dispatch)}
                >
                    Finish!
                </Button>
            </>
