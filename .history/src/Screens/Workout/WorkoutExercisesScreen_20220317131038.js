...
import { Text, StyleSheet, View, FlatList, TouchableOpacity, FAB } from 'react-native';
...
useEffect (() => {
    useEffect(() => {
        const clearExercises = navigation.addListener('blur', () => {
            setChosenExercises([]);
        });
...
return ( 
    <View>
        ...
        <FAB 
            style={styles.fab}
            icon='ios-add'
            disabled={chosenExercises.length > 0 ? true : false}
            onPress={() => {navigation.navigate('ExerciseDetail'), { params: { exercises: chosenExercises}}}}
        />
    </View>
);
};
