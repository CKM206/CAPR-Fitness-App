...
import { add_set, addSetToExercise, add_new_set } from '../workout/workoutSetsSlice';
    ...
    reducers: {
        ...
        extraReducers: (builder) => {
            builder
                ...
                .addCase(add_new_set, (state, action) => {
                    const { _id, exercise_id } = action.payload;

                    console.log('Payload From WorkoutExercises: ', action.payload);
                    //workoutExercisesAdapter.upsertOne(state, { _id: exercise_id, sets: _id })
                    state.entities[exercise_id].sets.push(_id);
                    //console.log('Added Set to Exercise');
                    //console.log(state.ids, state.entities)
                    console.log('State From WorkoutExercises: ', state);
                })        
    }
