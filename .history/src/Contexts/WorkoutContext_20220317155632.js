...
const workoutReducer = (state, action) => {
    switch (action.type) {
        ...
        case 'add_set':
            ...
            let newExerciseForSet= {   // Set Default State
                    exercise: {
                    name: action.payload.name,
                    exerciseType: action.payload.exerciseType,
                    muscles: action.payload.muscles,
                    ...
                    sets: [{
                        set: exerciseForSet.sets.length() + 1,
                        reps: 0,
                        weight: 0,
                        duration: 0,
                        rest: 0,
                ...
                allExercises.push(newExerciseForSet);


            console.log(allExercises);

            if (allExercises.filter(exercise => exercise._id === action.payload_id).sets[0].num === 0)
            {
                allExercises.filter(exercise => exercise._id === action.payload_id).sets.splice(0, 1);
            }
            
             return {...state, exercise: allExercises};
            //return {...state, sets: []};    
        case 'edit_title':
 ...
 */
 ...
 const addExercises = (dispatch) => {
    return async (arrayOfExercises) => {
        ...
        NavigationRef.navigate('WorkoutStack', {screen: 'WorkoutDetail', params: null})
